...
def add_skill():
    if request.method == 'POST':
        ...
        if hours_logged_str is None:
            return redirect(url_for('routes.dashboard'))  # Redirect if hours_logged is missing

        try:
            hours_logged = int(hours_logged_str)
        except ValueError:
            return redirect(url_for('routes.dashboard'))  # Redirect if hours_logged is not a valid integer
